...
export class CoelhoWalletService {
  ...
  public lastCurrency: Currency = {
    timeISO: "", BRL: 0, USD: 0, EUR: 0
  };
  private urlBRL = "http://api.coindesk.com/v1/bpi/currentprice/BRL.json";
  private urlUSDEUR = "http://api.coindesk.com/v1/bpi/currentprice.json";
  btcBalance = 0;
  ...
  public addBTCBalance(value: number) {
    const length = this.currencies.length;
    if (length > 0) {
  ...
  public removeBTCBalance(value: number) {
    const length = this.currencies.length;
    if (length > 0) {
      this.btcBalance -= value / this.lastCurrency.BRL;
    }
